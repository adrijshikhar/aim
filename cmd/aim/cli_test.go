...
func TestCLI_ExecuteRename(t *testing.T) {
	tempDir := t.TempDir()
	t.Setenv("AIM_HOME", tempDir)

	pm := profile.NewProfileManager(tempDir)
	origDir, err := pm.EnsureProfile("orig")
	if err != nil {
		t.Fatalf("failed to create profile 'orig': %v", err)
	}

	// Create tokens and custom config inside orig
	tokenDir := filepath.Join(origDir, ".gemini", "antigravity-cli")
	if err := os.MkdirAll(tokenDir, 0700); err != nil {
		t.Fatalf("failed to create token dir: %v", err)
	}
	if err := os.WriteFile(filepath.Join(tokenDir, "token.json"), []byte(`{"access_token":"secret123"}`), 0600); err != nil {
		t.Fatalf("failed to write token.json: %v", err)
	}
	if err := os.WriteFile(filepath.Join(origDir, "custom.conf"), []byte("env=prod"), 0600); err != nil {
	...
	cfg.AddProfileAgent("orig", "agy")
	cfg.AddProfileAgent("orig", "mock")
	cfg.SetProfileEnv("orig", map[string]string{"KEY": "VAL"})
	_ = config.SaveConfig(cfg)
	...
	// 1. Invalid usage with < 2 args or > 3 args
	if code := executeRename(reg, pm, []string{}); code != 1 {
		t.Errorf("expected exit code 1 for empty args, got %d", code)
	}
	if code := executeRename(reg, pm, []string{"orig"}); code != 1 {
		t.Errorf("expected exit code 1 for 1 arg, got %d", code)
	}
	if code := executeRename(reg, pm, []string{"1", "2", "3", "4"}); code != 1 {
		t.Errorf("expected exit code 1 for 4 args, got %d", code)
	}

	// 2. Rename unassociated agent
	if code := executeRename(reg, pm, []string{"unassociated", "orig", "new-dst"}); code != 1 {
		t.Errorf("expected exit code 1 for unassociated agent, got %d", code)
	}

	// 3. Rename non-existent profile
	if code := executeRename(reg, pm, []string{"ghost", "new-dst"}); code != 1 {
		t.Errorf("expected exit code 1 for non-existent profile, got %d", code)
	}

	// 4. Rename to same name
	if code := executeRename(reg, pm, []string{"orig", "orig"}); code != 1 {
		t.Errorf("expected exit code 1 for same-name rename, got %d", code)
	}

	// 5. Successful 2-argument rename: aim rename orig renamed-2arg
	if code := executeRename(reg, pm, []string{"orig", "renamed-2arg"}); code != 0 {
		t.Fatalf("expected exit code 0 for 2-arg rename, got %d", code)
	}

	// Old directory should not exist
	if _, err := os.Stat(origDir); !os.IsNotExist(err) {
		t.Errorf("expected old directory to not exist, got err: %v", err)
	}

	// New directory should exist
	newDir := pm.ProfileDir("renamed-2arg")
	if _, err := os.Stat(newDir); err != nil {
		t.Fatalf("expected new directory to exist: %v", err)
	}

	// Token MUST be preserved
	tokenFile := filepath.Join(newDir, ".gemini", "antigravity-cli", "token.json")
	if data, err := os.ReadFile(tokenFile); err != nil || !strings.Contains(string(data), "secret123") {
		t.Errorf("expected token.json to be preserved, got %s (err: %v)", string(data), err)
	}

	// Custom file MUST be preserved
	if data, err := os.ReadFile(filepath.Join(newDir, "custom.conf")); err != nil || string(data) != "env=prod" {
		t.Errorf("expected custom.conf to be preserved, got %s (err: %v)", string(data), err)
	}

	// Config updated
	cfgReload, _ := config.LoadConfig()
	if !cfgReload.HasAgent("renamed-2arg", "agy") || !cfgReload.HasAgent("renamed-2arg", "mock") {
		t.Errorf("expected renamed-2arg to have agy and mock, got %v", cfgReload.GetProfileAgents("renamed-2arg"))
	}
	if _, exists := cfgReload.Profiles["orig"]; exists {
		t.Errorf("expected orig to be removed from config")
	}
	if env := cfgReload.GetProfileEnv("renamed-2arg"); env["KEY"] != "VAL" {
		t.Errorf("expected env KEY=VAL to be preserved, got %v", env)
	}

	// Cache updated
	cacheReload := usage.NewCacheStore(tempDir, usage.DefaultTTL)
	if _, found := cacheReload.Get("agy", "renamed-2arg"); !found {
		t.Errorf("expected renamed-2arg to exist in usage cache")
	}
	if _, found := cacheReload.Get("agy", "orig"); found {
	...
	_, _ = pm.EnsureProfile("orig2")
	cfgReload.AddProfileAgent("orig2", "mock")
	_ = config.SaveConfig(cfgReload)

	if code := executeRename(reg, pm, []string{"mock", "orig2", "renamed-3arg"}); code != 0 {
		t.Fatalf("expected exit code 0 for 3-arg rename, got %d", code)
	}
	cfgReload, _ = config.LoadConfig()
	if _, exists := cfgReload.Profiles["renamed-3arg"]; !exists {
		t.Errorf("expected renamed-3arg in config")
	}

	// 8. Test runRename helper wrapper
	if err := runRename(reg, pm, []string{"renamed-3arg", "renamed-4arg"}); err != nil {
		t.Errorf("expected runRename to succeed, got: %v", err)
	}
	if err := runRename(reg, pm, []string{"nonexistent", "dst"}); err == nil {
		t.Errorf("expected runRename to fail for nonexistent profile")
	}

	// 9. Test shell completion
	renameCmd := newRenameCmd(reg, pm)
	if renameCmd.ValidArgsFunction == nil {
		t.Fatalf("expected ValidArgsFunction on renameCmd")
	}
	// Arg 0 completion: returns agents and all profiles
	completions, _ := renameCmd.ValidArgsFunction(renameCmd, []string{}, "")
	if len(completions) == 0 {
		t.Errorf("expected completions for arg 0, got none")
	}
	// Arg 1 completion with agent: returns profiles for that agent
	completionsAgent, _ := renameCmd.ValidArgsFunction(renameCmd, []string{"mock"}, "")
	if len(completionsAgent) == 0 {
		t.Errorf("expected completions for arg 1 with agent, got none")
	}
	// Arg 2 completion: returns nil
	completionsArg2, _ := renameCmd.ValidArgsFunction(renameCmd, []string{"mock", "orig"}, "")
	if completionsArg2 != nil {
		t.Errorf("expected nil completions for arg 2, got %v", completionsArg2)
	}
}
